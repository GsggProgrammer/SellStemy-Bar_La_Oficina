...
if (isset($_GET["boton"])) {
  ...
  if ($_GET["boton"] == "usuario_eli") {
      $objeto = new usuario(b: $_GET['doc']);
      $r = $objeto->Eliminacion();
      if ($r == 1) {
        echo "<script>
            if (confirm('El usuario ha sido eliminado')) {
                window.location.href = '../vista/VUsuario.html';
            }
        </script>";
    } else {
        echo "<script>
            if (confirm('El usuario no ha sido eliminado')) {
                window.location.href = '../vista/VUsuario.html';
            }
        </script>";
    }
              ...
              <div class="modal-dialog" role="document">
                <div class="modal-content rounded-modal">
                  <div class="modal-header">
                       <h2 class="modal-title" id="editProfileModalLabel">Editar Usuario</h2>
                     </div>
                     <form method="post" action="CUsuario.php">
